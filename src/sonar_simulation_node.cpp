...
int sonar_num = 12;
float sonarsBuf[12] = {0};
void sonarsCallback(const sensor_msgs::Imu& sonars_msg)
{
	unsigned char idx_ = 0;
	for(idx_ = 0; idx_ < 8; idx_ ++)
	{
		sonarsBuf[idx_] = (sonars_msg.orientation_covariance[idx_]+1.0)*0.22;
	}
	for(idx_ = 0; idx_ < 4; idx_ ++)
	{
		sonarsBuf[idx_ + 8] = (sonars_msg.angular_velocity_covariance[idx_]+1.0)*0.22;
	}
//	ROS_INFO("in");
...
//	bool increase_flag = true; // true - front sonar value increases, false - front sonar value decreases
	...
	while (n.ok())
	{
		...
		for(int idx_ = 0; idx_ < sonar_num; idx_ ++)
		{
